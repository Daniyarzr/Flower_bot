...
async def get_products_cached(category: str, min_p: int, max_p: int) -> List[Product]:
    ...
    # Cache miss or invalid: refresh from DB
    Session = get_sessionmaker()
    async with Session() as session:
        result = await session.execute(
            select(Product)
    ...
    logger.debug(f"Cache updated for key: {key}, products count: {len(products)}")
    return products
...
async def get_bot_text(key: str, default: str) -> str:
    """
    Retrieve customizable bot text from database or fallback to default.

    :param key: Text key (e.g., 'start_message')
    :param default: Default text if not found in DB
    :return: Text value
    """
    async with get_sessionmaker()() as session:
        result = await session.execute(select(BotText).where(BotText.key == key))
        bot_text_obj = result.scalar_one_or_none()
        return bot_text_obj.value if bot_text_obj else default


def delivery_human(delivery_type: Optional[str]) -> str:
    """Human-readable delivery type."""
    mapping = {
        "pickup": "🏃 Самовывоз",
...
async def show_product(c, category, min_p, max_p, index, price_data):
    """
    ...
    markup = kb_product_nav(category, f"{min_p}-{max_p}", index, len(products), product.id)
    ...
    if product.photo_file_id:
        ...
        if product.image_url.startswith(('http://', 'https://')):
            photo = product.image_url  # External URL
        else:
            # Assume local path relative to project root
            base = Path(__file__).resolve().parent.parent.parent  # Adjust levels if needed: app/handlers/user.py -> root
            path = base / product.image_url.lstrip('/')
            if path.exists():
                photo = FSInputFile(path)
            else:
                logger.warning(f"Local image path does not exist: {path}")

    # If photo exists, prefer sending/editing as photo
    if photo:
        ...
        try:
            if c.message.photo:
                ...
                # If current message is text, can't edit to photo, so send new and delete old
                await c.message.delete()
...
async def req_start(c: CallbackQuery, state: FSMContext):
    ...
    Saves product ID and prompts for delivery type.
    """
...
async def req_delivery(c: CallbackQuery, state: FSMContext):
    ...
    markup = kb_payment_type(is_pickup=is_pickup)
